...
// only resolves if all are resolve
...
// allSettled , runs all promises, regardless if one is error
Promise.allSettled(urls.map(url => fetch(url).then(people => people.json())))
  .then(array => {
    throw Error
    console.log('1', array[0])
    console.log('2', array[1])
